			...
			if( 0 == strncmp(p+section_start,section, i-section_start)) {
				...
				*sec_e = i;
				...
				&& !end_of_string(p[i]) ) {
					...
					while( !newline(p[i]) &&  !end_of_string(p[i]) &&!space(p[i])) {
						i++;
					}
...
{
	unsigned int sec_s,sec_e,key_s,key_e, value_s, value_e;
	if(!parse_file(aSection,aKey,aIniBuffer,&sec_s,&sec_e,&key_s,&key_e,&value_s,&value_e))
	{
		if(aDefaultValue!=NULL) strncpy(aValue,aDefaultValue, aValueLength);
		return 0;
	}
	else
	{
		unsigned int  length = value_e -value_s;
		if(length>aValueLength-1) length = aValueLength-1;
		memset(aValue, 0, aValueLength);
		memcpy(aValue,aIniBuffer+value_s, length );
		aValue[length] = '\0';
		return 1;
	}
}
...
{
	char value[32] = {0};
	if(!read_inistring_string(aIniBuffer,aSection,aKey,value, sizeof(value),NULL))
